/* ═══════════════════════════════════════════════════════════
   CC PRO BAU — SCRIPT.JS (pagina principală)
   Vanilla JS, fără dependențe.
   ═══════════════════════════════════════════════════════════ */
...
(function () {
    ...
    /* Limitează frecvența unui handler la un cadru de randare */
    ...
    function initCounters() {
        ...
        function animate(el) {
            ...
            function step(now) {
                ...
                t < 1 ? window.requestAnimationFrame(step) : setFinal(el);
            }
    ...
    function initBeforeAfter() {
        ...
        $$("[data-ba]").forEach((wrap) => {
            ...
            /* Ascunde cardul dacă imaginile lipsesc */
        ...
        filters.forEach((btn) => {
            btn.addEventListener("click", () => {
                ...
                $$(".ba-card", grid).forEach((card) => {
                    const show = value === "all" || card.dataset.baCat === value;
    ...
    const VIDEOS = [
       {
            src: "meta/new_data/b8be93c3-fad7-46bb-818f-fed510d2e622.mov",
        ...
        },
        {
            ...
            desc: "Profesionalism garantat pe fiecare șantier."
        },
        {
    ...
    /* Generează poster din primul cadru al videoclipului */
    ...
    function initPlayer() {
        ...
        /* Fără clipuri → ascundem secțiunea */
        if (!VIDEOS.length) {
        ...
        const broken = new Set();     // clipuri care au dat eroare
        ...
        VIDEOS.forEach((item, i) => {
            ...
            /* Miniatură: poster propriu sau generată din video */
        ...
        /* — Clip următor / gestionarea erorilor — */
        ...
        function handleAllBroken() {
            console.warn("[CC Pro Bau] Niciun videoclip disponibil.");
    ...
    function initMascot() {
        ...
        const img    = char ? $("img", char) : null;

        /* Dacă imaginea lipsește, scoatem tot */
        if (img) {
            img.addEventListener("error", () => {
                console.warn("[Mascotă] Imagine negăsită:", img.src);
                wrap.remove();
            }, { once: true });
        }
